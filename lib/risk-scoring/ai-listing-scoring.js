...
async function logTrustScoreChange(
  ...
  try {
    // Créer la table trust_score_history si elle n'existe pas
    // Note: Tu devras ajouter ce modèle dans ton schema.prisma
...
async function updateListingTrustScore(listingId, forceRefresh = false) {
  try {
    ...
    // Récupérer les reviews depuis les bookings
    const allReviews = listing.bookings
      .filter((b) => b.review && b.status === "COMPLETED")
      .map((b) => b.review);
...
Annonce bloquée: ${listing.isBlocked ? "" : ""}
`;
    ...
    let finalScore = result?.listingScore || 50;
    ...
    // 8. Ajouter les flags
    const scamFlags = result?.scamFlags || [];
