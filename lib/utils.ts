...
export function isClerkAPIError(
  error: unknown,
): error is ClerkAPIErrorResponse {
...
export const ValidationPatterns = {
  ...
  validateIdentifier: (value: string): boolean => {
    ...
    return (
      ValidationPatterns.isEmail(value) || ValidationPatterns.isUsername(value)
    );
  },
};
...
export type ClerkLoginErrorCode =
  (typeof ClerkLoginErrorCodes)[keyof typeof ClerkLoginErrorCodes];
...
export function getClerkErrorMessage(
  ...
  if (!isClerkAPIError(error)) {
    if (isStandardError(error)) {
      ...
      if (
        error.message &&
        (error.message.includes("Veuillez") ||
          error.message.includes("réessayer"))
      ) {
...
export function isRateLimitError(error: unknown): boolean {
  ...
  const code = error.errors[0]?.code;
  return (
    code === ClerkLoginErrorCodes.TOO_MANY_ATTEMPTS ||
