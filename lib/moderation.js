/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║   NESTHUB — MOTEUR DE MODÉRATION 100% IA (GPT-4o mini)                   ║
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 RÈGLES DE DISTINCTION (anti-faux-positifs) :
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
mentions "numéro/raqmi/رقمي/WA"
 ATTENTION : Les nombres de 1 à 4 chiffres seuls (comme "53", "56", "10", "50") 
...
Ils ne deviennent des téléphones que lorsqu'ils sont concaténés sur plusieurs messages.
 FRAGMENTATION CROSS-MESSAGE (sur plusieurs messages) :
...
(OK pour nesthub.com, nesthub.tn uniquement)
 ...
 gps : lat/long, DMS, what3words, liens maps, "position"
 ...
 real_estate_scam : "étranger" + "clés DHL", "msakin dawla", prix irréaliste (50 TND)
...
const UI_MESSAGES = {
  ...
  security: {
    ...
    noAdmin:
      " L'équipe NestHub ne vous demandera jamais de code OTP, mot de passe ou coordonnées bancaires par message.",
  },
  report: {
    success:
...
function checkRateLimit(userId) {
  ...
  if (userData.count > 100) {
    console.warn(` Rate limit atteint pour user ${userId}`);
    return false;
  }
...
//   FALLBACK ROBUSTE — quand l'IA est DOWN
...
//   FONCTION PRINCIPALE — AVEC CACHE ET MÉTRIQUES
...
async function moderateMessage(content, userId = null) {
  ...
  try {
    ...
    if (cachedAIResult) {
      console.log(` [AI CACHE] Hit pour message: ${sanitized.slice(0, 30)}...`);
    ...
    if (fallback.isBlocked) {
      console.log(
        ` [FALLBACK] Bloqué : ${fallback.category} | "${fallback.detected}"`,
      );
      const finalResult = {
        isBlocked: true,
        reason: `${fallback.reason}`,
    ...
    console.warn(
      ` [FALLBACK] IA down, message non bloqué : "${sanitized.slice(0, 50)}"`,
    );
    const finalResult = {
      isBlocked: false,
      reason: null,
