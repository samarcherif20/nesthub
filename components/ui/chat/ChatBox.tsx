    ...
    <div className="relative flex-shrink-0 group">
      <div className="flex -space-x-2">
        {visibleParticipants.map((p, idx) => (
          <div
            key={p.id}
            className="relative rounded-full overflow-hidden border-2 border-white dark:border-slate-900 shadow-md"
            style={{
              width: size,
              height: size,
        ...
        {remainingCount > 0 && (
          <div
            className="relative rounded-full bg-purple-500 text-white text-xs font-bold flex items-center justify-center border-2 border-white dark:border-slate-900 shadow-md"
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-white/50">{t("voice.loading")}</span>
      </div>
    );
  }
  ...
  if (!audioSrc) {
    return (
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-white/70">{t("voice.unavailable")}</span>
      </div>
    );
  }
...
function SystemMessage({
  ...
  return (
    <div className="flex justify-center my-2">
      <div className="max-w-[85%] px-4 py-2.5 bg-indigo-50 dark:bg-indigo-950/30 rounded-2xl border border-indigo-100 dark:border-indigo-800/50">
        <div className="flex items-center justify-center gap-2 text-center">
          {getMessageIcon(displayContent)}
...
function OfferCard({
  ...
  const checkInDate = checkIn ? formatDate(checkIn) : t("offer.dateNotDefined");
  const checkOutDate = checkOut
    ? formatDate(checkOut)
    : t("offer.dateNotDefined");
  const nights =
    checkIn && checkOut
      ? Math.ceil(
          ...
          <div className="absolute bottom-0 left-0 right-0 p-5 text-white z-10">
            ...
            <div className="flex items-center gap-4 mt-3">
              ...
              <p className="text-xs text-white/90 flex items-center gap-1 drop-shadow-md">
                ...
                {guests}{" "}
                {guests > 1 ? t("offer.travelers") : t("offer.traveler")}
              </p>
            </div>
...
export function ChatBox({
 ...
 useEffect(() => {
  ...
  const handleOpenBlockSheet = () => {
    if (blockCooldown) {
      showToastMsg(t("block.cooldown", { hours: cooldownRemaining }), "error");
      return;
    }
  ...
  const confirmBlock = async () => {
    ...
    showToastMsg(t("block.userBlocked", { name: recipientName }), "error");
  };
    ...
    if (confirm(t("confirm.delete"))) {
      try {
        ...
        if (res.ok) {
          ...
          showToastMsg(t("toast.messageDeleted"), "success");
        } else showToastMsg(t("toast.deleteError"), "error");
      } catch {
        showToastMsg(t("toast.connectionError"), "error");
      }
    }
  };
  ...
  const handleSend = async () => {
        ...
        if (availabilityRes.ok) {
          ...
          if (availability.length > 0) {
            ...
            if (!todaySlot || !todaySlot.enabled) {
              isAvailable = false;
              unavailableMessage = t("availability.notAvailableToday");
            } else if (todaySlot.hours === "Fermé") {
              isAvailable = false;
              unavailableMessage = t("availability.closedToday");
              ...
              if (hoursMatch) {
                ...
                if (currentTotalMinutes < startTotalMinutes) {
                  isAvailable = false;
                  unavailableMessage = t("availability.availableAt", {
    ...
    if (isGroup) {
      ...
      try {
        const res = await fetch(
          `/api/conversations/groups/${cleanId}/messages`,
          {
            method: "POST",
        ...
        if (res.ok) {
          ...
          showToastMsg(error.error || t("toast.sendError"), "error");
        }
      } catch (error) {
  ...
  const handleSendVoice = async (audioBlob: Blob, duration: number) => {
    ...
    try {
      ...
      if (uploadRes.ok) {
        ...
        showToastMsg(t("toast.voiceSent"), "success");
      } else {
  ...
  const handleReportConversation = async () => {
    ...
    if (now - lastReportTime < 5 * 60 * 1000) {
      showToastMsg(t("report.cooldown"), "error");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/conversations/${conversationId}/report`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reason: t("report.defaultReason") }),
      });
      if (res.ok) {
        setLastReportTime(now);
        showToastMsg(t("report.success"), "success");
      } else showToastMsg(t("report.error"), "error");
    } catch {
      showToastMsg(t("toast.connectionError"), "error");
    }
    setShowMoreMenu(false);
  ...
  const handleBlockUser = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const systemMsgRes = await fetch(
          `/api/conversations/${conversationId}/messages`,
          {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              content: t("block.systemMessageBlock", { name: recipientName }),
              isSystem: true,
            }),
          },
        );
      ...
      } else showToastMsg(t("block.blockError"), "error");
    } catch {
      showToastMsg(t("toast.connectionError"), "error");
    }
    setShowMoreMenu(false);
  ...
  const handleUnblockUser = async () => {
    try {
      ...
      if (res.ok) {
        ...
        showToastMsg(
          t("block.userUnblocked", { name: recipientName }),
          "success",
        );
        ...
        const systemMsgRes = await fetch(
          `/api/conversations/${conversationId}/messages`,
          {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              content: t("block.systemMessageUnblock", { name: recipientName }),
              isSystem: true,
            }),
          },
        );
        ...
        showToastMsg(t("block.unblockError"), "error");
      }
    } catch (error) {
  ...
  const handleConfirmOffer = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        showToastMsg(t("offer.created"), "success");
        setShowOfferCard(false);
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 overflow-hidden relative">
          ...
          <div className=" backdrop-blur-md rounded-2xl p-4 flex items-center gap-4 shadow-[0_8px_30px_-6px_rgba(79,70,229,0.2)] border border-red-200/50 dark:border-red-800/30 hover:shadow-[0_12px_40px_-8px_rgba(79,70,229,0.3)] transition-all duration-300">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-700 dark:text-gray-400 leading-relaxed">
                {blockReason || t("safety.defaultMessage")}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mx-4 mt-2 mb-1 p-3 bg-slate-100 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-600 dark:text-slate-400">
              {t("block.youAreBlocked", { name: recipientName })}
            </span>
          </div>
        </div>
      )}

      {/* Info panel for owner */}
      ...
      {blockedNotification && (
        <div className="mx-3 mt-2 flex items-center gap-2 p-2.5 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-xl shrink-0">
          ...
          <span className="text-xs text-amber-700 dark:text-amber-400">
            {t("block.messageBlocked")} — {blockedNotification}
          </span>
        </div>
      )}
        ...
        {messages.length === 0 && !showOfferCard ? (
          ...
          <>
            {messages.map((msg) => {
              ...
              // 2. AI BLOCKED MESSAGES
              if (isIAModerated) {
                const badgePosition = isOwn
                  ? "-top-3 -left-3"
                  : "-top-3 -right-3";
                const blockReasonMsg = (msg as any).reason || "";
                    ...
                    {msg.replyTo && (
                      <div className="mb-1.5 pb-1.5 border-b border-white/20 dark:border-slate-700 text-[10px] text-white/60 dark:text-slate-400">
                        <span className="font-medium">
                          {t("reply.replyTo")} {msg.replyTo.senderName}:
                        </span>
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h3 className="font-semibold text-base text-slate-900 dark:text-white">
                    {t("block.blockTitle", { name: recipientName })}
                  </h3>
      ...
      {!isAdminLocked ? (
        hasBlockedUser ? (
          /* User has blocked the recipient */
          <div className="px-4 py-6 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
            <div className="flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
                {t("block.youBlocked", { name: recipientName })}
              </p>
              <p className="text-xs text-slate-400 mt-1 mb-4">
                {t("block.noMoreMessages")}
              </p>
              <button
          ...
          <div className="px-3 py-3 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
            ...
            <div className="flex items-end gap-2 bg-slate-50 dark:bg-slate-800 rounded-xl px-3 py-2 border border-slate-200 dark:border-slate-700 focus-within:border-indigo-300 transition-colors">
              ...
              <textarea
                ...
                placeholder={
                  replyTo
                    ? t("input.replyPlaceholder", { name: replyTo.senderName })
                    : t("input.placeholder")
                }
                rows={1}
                disabled={
            ...
            <p className="text-[10px] text-slate-400 text-center mt-2 flex items-center justify-center gap-1">
              ...
              {t("footer.secureMessages")}
            </p>
          </div>
        )
      ) : (
        /* Footer locked by admin */
