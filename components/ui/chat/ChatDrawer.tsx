...
export function ChatDrawer({ isOpen, onClose, userRole }: ChatDrawerProps) {
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-[200] bg-black/40 dark:bg-black/60 backdrop-blur-[2px]"
        onClick={onClose}
      />
      ...
      <div className="fixed right-0 top-0 h-full z-[201] w-full max-w-[380px] flex flex-col bg-white dark:bg-slate-950 border-l border-gray-100 dark:border-slate-800 shadow-2xl animate-in slide-in-from-right duration-250">
        ...
        <div className="shrink-0 px-5 pt-5 pb-4 border-b border-gray-100 dark:border-slate-800">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="w-[30px] h-[30px] rounded-full bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 flex items-center justify-center text-gray-500"
            >
              <IconClose />
            </button>
          </div>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 px-5 py-3.5 border-b border-gray-100 dark:border-slate-800/80 transition-colors group ${
                      unread
                        ? "bg-blue-50/30 dark:bg-blue-900/10 hover:bg-white"
                        : "hover:bg-white"
                    }`}
                  >
                    <Avatar
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline justify-between gap-2 mb-0.5">
                        <span
                          className={`text-[13px] truncate ${unread ? "font-bold text-gray-900" : "font-medium text-gray-700"}`}
                        >
                          {username}
                        </span>
                        <span
                          className={`text-[10px] shrink-0 ${unread ? "font-semibold text-blue-600" : "text-gray-400"}`}
                        >
                      ...
                      {conv.lastMessage && (
                        <p
                          className={`text-[11px] truncate ${unread ? "text-gray-600 font-medium" : "text-gray-400"}`}
                        >
        ...
        <div className="shrink-0 px-4 py-3.5 border-t border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-950">
          <Link
            href={allLink}
            onClick={onClose}
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-blue-50 dark:bg-blue-950/40 hover:bg-blue-100 text-blue-700 font-semibold text-[13px]"
          >
            <BiSolidMessageSquareAdd /> {t("footer.viewAll")}{" "}
            <IconChevronRight />
          </Link>
