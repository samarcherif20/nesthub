...
import { ChevronLeft, ChevronRight, Clock, LockOpen, MousePointerClick } from "lucide-react";
import {
  RiCalendarCheckLine, RiLockLine, RiMoneyDollarCircleLine,
  RiCheckboxCircleLine,
} from "react-icons/ri";
import { TbCalendarOff, TbCalendarCheck, TbCalendarSearch } from "react-icons/tb";
...
.sc-pill{ border-radius:4px; padding:2px 5px; font-size:9px; font-weight:700; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; cursor:pointer; transition:opacity .12s; display:flex;align-items:center;gap:3px; }
.sc-pill:hover{ opacity:.85; }
...
const COLORS = {
  booking: {
    ...
    text:  "#fff",
    light: (d: boolean) => d ? "rgba(99,102,241,.20)" : "rgba(99,102,241,.12)",
  },
  blocked: {
  ...
  price: {
    ...
    light: (d: boolean) => d ? "rgba(245,158,11,.20)" : "rgba(245,158,11,.12)",
  },
};

function getEventInfo(day: any, dark: boolean, t: any) {
  if (day.isBooked)
    return { type:"booking", bg:COLORS.booking.bg(dark), text:COLORS.booking.text, label: day.bookingGuest || t("booking") };
  if (day.isBlocked && !day.isPast)
    return { type:"blocked", bg:COLORS.blocked.bg(dark), text:COLORS.blocked.text, label: day.blockedReason || t("blocked") };
  if (day.customPrice && !day.isPast)
    return { type:"price",   bg:COLORS.price.bg(dark),   text:COLORS.price.text(dark), label:`${day.customPrice} TND` };
  return null;
}

// ── component ─────────────────────────────────────────────────────────────────
export function SimpleCalendar({
  days, currentDate, onDateChange, onDateClick, selectedDates, onDateMouseEnter, isDark = false,
}: {
  days: any[]; currentDate: Date; onDateChange: (d: Date) => void;
  onDateClick: (day: any) => void; selectedDates: Date[];
  onDateMouseEnter?: (day: any) => void; isDark?: boolean;
}) {
  ...
  const changeMonth = (d: number) => { const n = new Date(currentDate); n.setMonth(n.getMonth()+d); onDateChange(n); };
  const changeWeek  = (d: number) => { const n = new Date(currentDate); n.setDate(n.getDate()+d*7); onDateChange(n); };
  ...
  const renderMonth = () => (
    <div className="sc-grid-wrap">
      <div className="sc-col-headers">
        {FR_DAYS_SHORT.map(d => <div key={d} className="sc-col-hdr">{d}</div>)}
      </div>
        ...
        {days.map((day, i) => {
          ...
          return (
            <div key={i} className={cls}
              onClick={() => !day.isPast && day.isCurrentMonth && onDateClick(day)}
              onMouseEnter={() => onDateMouseEnter?.(day)}>
              <span className="sc-num">{day.date.getDate()}</span>
              <div className="sc-events">
                {/* selected pill — shown when no other event */}
                {isSd && !ev && (
                  <div className="sc-pill sel-pill">
                    <RiCheckboxCircleLine style={{ width:9, height:9, flexShrink:0 }}/>
                    {t("selected")}
                  </div>
                )}
                {ev && (
                  <div className="sc-pill" style={{ background:ev.bg, color:ev.text }}>
                    {ev.type==="booking" && <RiCalendarCheckLine    style={{ width:9, height:9, flexShrink:0 }}/>}
                    {ev.type==="blocked" && <RiLockLine             style={{ width:9, height:9, flexShrink:0 }}/>}
                    {ev.type==="price"   && <RiMoneyDollarCircleLine style={{ width:9, height:9, flexShrink:0 }}/>}
    ...
    <div className="sc-week-wrap">
      <div className="sc-week-inner sc-grid-wrap">
        ...
        <div className="sc-week-header">
          <div className="sc-week-hcell" style={{ display:"flex", alignItems:"center", justifyContent:"center" }}>
          ...
          {weekDates.map((wd, i) => {
            ...
            return (
              <div key={i} className="sc-week-hcell">
                ...
                <div className={`sc-week-hnum${isTd?" today":""}${isSd&&!isTd?" sel-day":""}`}>
                  {wd.getDate()}
                </div>
              </div>
            );
          ...
          {hours.map(h => {
            ...
            return (
              <div key={h} className="sc-hour-row">
                <div className={`sc-hour-lbl${h===nowHour&&isSameDay(new Date(),today)?" now-lbl":""}`}>
                ...
                {weekDates.map((wd, di) => {
                  ...
                  return (
                    <div key={di} className={cellCls} style={{ position:"relative" }}>
                      {nowRow && isSameDay(wd, today) && (
                        <div className="sc-now-line" style={{ top:`${topPct}%` }}>
                          <div className="sc-now-dot"/>
                        </div>
                      )}
                      <button className="sc-hour-btn"
                        disabled={!day || day.isPast || isBkd || isBlk}
                        onClick={() => day && onDateClick(day)}
                        onMouseEnter={() => day && onDateMouseEnter?.(day)}>
                        {isBkd && h===8 && (
                          <div className="sc-hour-pill" style={{ background:COLORS.booking.bg(isDark), color:COLORS.booking.text }}>
                            <RiCalendarCheckLine style={{ width:9, height:9 }}/> {day?.bookingGuest||t("booking")}
                          </div>
                        )}
                        {isBlk && h===8 && (
                          <div className="sc-hour-pill" style={{ background:COLORS.blocked.bg(isDark), color:COLORS.blocked.text }}>
                            <RiLockLine style={{ width:9, height:9 }}/> {day?.blockedReason||t("blocked")}
                          </div>
                        )}
                        {isPrc && h===8 && (
                          <div className="sc-hour-pill" style={{ background:COLORS.price.bg(isDark), color:COLORS.price.text(isDark) as string }}>
                            <RiMoneyDollarCircleLine style={{ width:9, height:9 }}/> {day?.customPrice} TND
                          </div>
                        )}
                        {isSd && !isBlk && !isBkd && !isPrc && h===8 && (
                          <div className="sc-hour-pill" style={{ background:"rgba(99,102,241,.18)", color:"#6366f1" }}>
                            <RiCheckboxCircleLine style={{ width:9, height:9 }}/> {t("selected")}
                          </div>
                        )}
                      </button>
                    </div>
  ...
  // ── LIST ────────────────────────────────────────────────────────────────────
  const renderList = () => {
    const evDays = days.filter(d => d.isCurrentMonth).sort((a,b) => a.date.getTime()-b.date.getTime());
    const groups: { label:string; items:any[] }[] = [];
    evDays.forEach(d => {
      const label = d.date.toLocaleDateString("fr-FR", { weekday:"long", day:"2-digit", month:"long" });
      ...
      <div className="sc-grid-wrap">
        {groups.length===0 && (
          <div className="sc-list-empty">
            <TbCalendarSearch style={{ width:28, height:28, margin:"0 auto 8px", color:"var(--c-head-txt)" }}/>
            {t("noEvents")}
          </div>
        )}
            ...
            {groups.map(({ label, items }) => (
              <React.Fragment key={label}>
                <tr className="sc-list-dh">
                  <td colSpan={4} style={{ textTransform:"capitalize" }}>{label}</td>
                </tr>
                {items.map((day, i) => {
                  const ev  = getEventInfo(day, isDark, t);
                  const sel = isSel(day.date);
                  return (
                    <tr key={i} className={`sc-list-row${sel?" sel-row":""}`}
                      ...
                      <td style={{ width:24, paddingRight:0 }}>
                        {(ev || sel) && (
                          <span style={{ display:"inline-block", width:10, height:10, borderRadius:3,
                            background: ev ? ev.bg : "#6366f1", verticalAlign:"middle" }}/>
                        )}
                      </td>
                      {/* pills */}
                      <td>
                        {sel && (
                          <div className="sc-status-pill" style={{ background:"rgba(99,102,241,.12)", color:"#6366f1", marginRight:6, display:"inline-flex" }}>
                            <RiCheckboxCircleLine style={{ width:11, height:11 }}/> {t("selected")}
                          </div>
                        )}
                        {day.isBooked && (
                          <div className="sc-status-pill" style={{ background:COLORS.booking.light(isDark), color:COLORS.booking.bg(isDark) }}>
                            <RiCalendarCheckLine style={{ width:11, height:11 }}/> {day.bookingGuest||t("booking")}
                          </div>
                        )}
                        {day.isBlocked && !day.isPast && (
                          <div className="sc-status-pill" style={{ background:COLORS.blocked.light(isDark), color:COLORS.blocked.bg(isDark) }}>
                            <RiLockLine style={{ width:11, height:11 }}/> {day.blockedReason||t("blocked")}
                          </div>
                        )}
                        {day.customPrice && !day.isBlocked && !day.isBooked && !day.isPast && (
                          <div className="sc-status-pill" style={{ background:COLORS.price.light(isDark), color:COLORS.price.bg(isDark) }}>
                            <RiMoneyDollarCircleLine style={{ width:11, height:11 }}/> {t("specialPrice")}
                          </div>
                        )}
                        {!day.isBooked && !day.isBlocked && !day.customPrice && !day.isPast && !sel && (
                          <div className="sc-status-pill" style={{ background:"rgba(16,185,129,.10)", color:"#059669" }}>
                            <TbCalendarCheck style={{ width:11, height:11 }}/> {t("available")}
                          </div>
                        )}
                        {day.isPast && (
                          <div className="sc-status-pill" style={{ background:"var(--c-btn-bg)", color:"var(--c-head-txt)" }}>
                            <TbCalendarOff style={{ width:11, height:11 }}/> {t("past")}
                          </div>
                        )}
                      </td>
                      {/* price */}
                      <td style={{ textAlign:"right", fontWeight:700, color: day.customPrice?"#f59e0b":"var(--c-head-txt)" }}>
                        ...
                        {!day.isPast && !day.isBooked && !day.isBlocked && (
                          <button onClick={e => { e.stopPropagation(); onDateClick(day); }}
                            style={{ fontSize:10, fontWeight:700, padding:"3px 9px", borderRadius:6, border:"none", cursor:"pointer",
                              background: sel ? "#6366f1" : "rgba(99,102,241,.10)",
    ...
    <div className={`sc${isDark?" dark":""}`}>
      ...
      <div className="sc-toolbar">
        <div className="sc-nav">
          <button className="sc-ico-btn" onClick={handlePrev}><ChevronLeft size={16}/></button>
          <button className="sc-ico-btn" onClick={handleNext}><ChevronRight size={16}/></button>
          <button className="sc-today-btn" onClick={goToday}>{t("today")}</button>
        </div>
        <div className="sc-title">{title}</div>
        <div className="sc-toggle">
          {VIEWS.map(({ val, Icon, key }) => (
            <button key={val} className={`sc-vbtn${view===val?" on":""}`} onClick={() => setView(val)}>
      ...
      <div className="sc-legend">
        <div className="sc-leg-item">
          ...
          <RiCalendarCheckLine style={{ width:10, height:10 }}/> {t("legendBooked")}
        </div>
        <div className="sc-leg-item">
        ...
        <div className="sc-leg-item">
          ...
          <RiMoneyDollarCircleLine style={{ width:10, height:10 }}/> {t("legendSpecialPrice")}
        </div>
