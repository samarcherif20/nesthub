...
import {
  ...
  Star, TrendingUp, Users, Wallet, X, Sparkles,
  Calendar,
  Calendar1Icon,
  CalendarIcon
} from "lucide-react";
...
export default function Reservations() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-indigo-50 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950">
      ...
      {toast && (
        <div className="fixed top-20 left-1/2 z-[80] -translate-x-1/2 rounded-full bg-slate-900/92 px-6 py-3.5 text-sm font-bold text-white shadow-2xl backdrop-blur-xl dark:bg-white/92 dark:text-slate-900">
          {toast}
        </div>
      )}
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-10">
        ...
        <div className="mb-10">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/75 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.22em] text-indigo-600 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-slate-900/70 dark:text-indigo-300">
            <CalendarDays className="h-3.5 w-3.5 fill-indigo-600 text-indigo-600 dark:fill-indigo-300 dark:text-indigo-300" />
            MES SÉJOURS
          </div>
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white md:text-6xl">
            Mes <span className={GRADIENT_TEXT}>réservations</span>
          </h1>
          <p className="mt-2 max-w-xl text-sm leading-relaxed text-slate-500 dark:text-slate-400 md:text-base">
            Gérez et suivez tous vos séjours en un seul endroit. Consultez vos réservations à venir, vos séjours passés et vos annulations.
          </p>
        </div>
          ...
          <div className="relative grid gap-0 divide-y divide-slate-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4 dark:divide-white/10">
            {[
              ...
              <div key={l} className="flex items-center gap-4 px-6 py-6">
                <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${g} text-white shadow-md`}>
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="overflow-x-auto pb-1">
            <div className="flex items-center gap-2">
              {[
                { k: "UPCOMING" as const, l: "À venir", n: upcoming.length, icon: CalendarDays },
                { k: "PAST" as const, l: "Terminées", n: past.length, icon: CheckCircle2 },
                { k: "CANCELLED" as const, l: "Annulées", n: cancelled.length, icon: Ban }
              ].map(({ k, l, n, icon: Icon }) => {
                const active = tab === k;
                return (
                  <button
          ...
          <select
            ...
            className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 outline-none dark:border-white/10 dark:bg-white/5 dark:text-slate-200"
          >
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
          ...
          <div className="space-y-5">
            {/* Empty state - with dotted card same height as both aside cards combined */}
            {!hasBookings ? (
              <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-white/50 p-12 text-center dark:border-white/20 dark:bg-slate-900/30" style={{ minHeight: "calc(320px + 340px)" }}>
                ...
                <p className="mt-2 max-w-md text-sm leading-relaxed text-slate-500 dark:text-slate-400">
                  Explorez les plus belles demeures de Tunisie et réservez votre prochain séjour.
                  Elles apparaîtront ici, prêtes à être gérées.
                </p>
                <Link
                  href="/fr/search"
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-white/50 p-12 text-center dark:border-white/20 dark:bg-slate-900/30">
                <div className="mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-sky-500/10 via-indigo-500/10 to-purple-600/10 backdrop-blur-sm mx-auto">
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  Essayez une autre catégorie pour voir vos réservations.
                </p>
                <button
                  type="button"
                      ...
                      <div className="flex flex-col justify-between flex-1 p-5">
                        <div>
                          ...
                          <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-2.5 dark:bg-white/5">
                            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 via-purple-500 to-indigo-600 text-[11px] font-bold text-white shadow-md shadow-indigo-500/15">
                              {b.host.initials}
                            </div>
                            ...
                            <div className="mt-4 grid gap-4 rounded-xl bg-slate-50 p-4 sm:grid-cols-2 dark:bg-white/5">
                              <div className="space-y-2 text-sm">
                                ...
                                <div className="flex justify-between border-t border-slate-200 pt-2 text-base font-extrabold dark:border-white/10">
                                  <span className={GRADIENT_TEXT}>Total</span>
                                  <span className={GRADIENT_TEXT}>{b.totalPrice.toLocaleString()} TND</span>
                                </div>
                              </div>
                              <div>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {b.status === "PENDING" && (
                            <button className="flex-1 rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs font-bold text-slate-700 hover:border-indigo-200 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
                          ...
                          {isActive && (
                            <>
                              ...
                              <button className={`flex-1 rounded-xl ${GRADIENT_BUTTON} px-4 py-3 text-xs font-bold text-white shadow-md shadow-indigo-500/15 hover:scale-[1.01]`}>
          ...
          <aside className="space-y-5">
            <div className="sticky top-24 space-y-5">
              {/* CTA Card */}
              <div className="relative overflow-hidden rounded-2xl border border-white/70 bg-white/80 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-slate-900/80">
                <div className="absolute inset-0 bg-gradient-to-br from-sky-500 via-purple-500 to-purple-600" />
                <div className="relative p-6 text-center text-white">
                  <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-xl bg-white/15 backdrop-blur-sm">
                    <Compass className="h-7 w-7" />
                  </div>
                  <h3 className="text-lg font-extrabold tracking-tight">Prêt pour l'évasion ?</h3>
                  <p className="mt-1 text-xs leading-relaxed text-white/70">Des villas de luxe aux dars de charme, trouvez votre prochaine destination.</p>
                  <Link href="/fr/search">
                    <button className="mt-4 w-full rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-indigo-600 shadow-lg transition-all hover:scale-[1.01]">
                      Explorer les biens <ArrowRight className="ml-1 inline h-3.5 w-3.5" />
                    </button>
                  </Link>
                </div>
              </div>
              ...
              <div className={`${CARD_STYLE} p-5`}>
                ...
                <button className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 shadow-sm hover:border-indigo-200 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
          ...
          <section className="mt-16">
            <div className="mb-8">
              ...
              <h2 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white md:text-5xl">
                Inspirations pour <span className={GRADIENT_TEXT}>vous.</span>
              </h2>
            </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <Link href={`/fr/listings/${recommendations[0].id}`}>
                      <button className={`rounded-full ${GRADIENT_BUTTON} px-5 py-2.5 text-xs font-bold text-white shadow-lg shadow-indigo-500/20 transition-all hover:scale-105`}>
              ...
              <div className="group relative overflow-hidden rounded-3xl border border-white/70 bg-white/85 shadow-sm backdrop-blur-md transition-all hover:-translate-y-2 hover:shadow-[0_24px_60px_rgba(15,23,42,0.12)] dark:border-white/10 dark:bg-slate-900/85 h-[230px]">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 z-10">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <Link href={`/fr/listings/${recommendations[1].id}`}>
                      <button className={`rounded-full ${GRADIENT_BUTTON} px-3 py-1.5 text-[10px] font-bold text-white shadow-lg shadow-indigo-500/20 transition-all hover:scale-105`}>
                        Voir
                      </button>
                    </Link>
                  </div>
              ...
              <div className="group relative overflow-hidden rounded-3xl border border-white/70 bg-white/85 shadow-sm backdrop-blur-md transition-all hover:-translate-y-2 hover:shadow-[0_24px_60px_rgba(15,23,42,0.12)] dark:border-white/10 dark:bg-slate-900/85 h-[230px]">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 z-10">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <Link href={`/fr/listings/${recommendations[2].id}`}>
                      <button className={`rounded-full ${GRADIENT_BUTTON} px-3 py-1.5 text-[10px] font-bold text-white shadow-lg shadow-indigo-500/20 transition-all hover:scale-105`}>
                        Voir
                      </button>
                    </Link>
