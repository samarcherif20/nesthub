    ...
    <div className="flex items-center gap-3">
      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-100 to-violet-100 dark:from-indigo-900/50 dark:to-violet-900/50 flex items-center justify-center overflow-hidden ring-2 ring-indigo-500/10 flex-shrink-0">
        {owner?.profilePictureUrl && !err ? (
          <img
            src={`/api/listings/image?url=${encodeURIComponent(owner.profilePictureUrl)}`}
            alt=""
            className="w-full h-full object-cover"
            onError={() => setErr(true)}
          />
...
export default function AdminPropertiesPage() {
  ...
  const displayedAlerts = showAllAlertsState
    ? recentAlerts
    : recentAlerts?.slice(0, 4);
  ...
  const getSortIcon = (field: SortField) => {
    ...
    return sortOrder === "asc" ? (
      <IoArrowUp className="inline ml-1 text-xs" />
    ) : (
      <IoArrowDown className="inline ml-1 text-xs" />
    );
  };
  ...
  const handleActionWithConfirm = (action: string, id?: string) => {
    ...
    if (action === "activer") mappedAction = id ? "ACTIVATE" : "BULK_ACTIVATE";
      ...
      mappedAction = id ? "DEACTIVATE" : "BULK_DEACTIVATE";
    else if (action === "archiver")
      mappedAction = id ? "ARCHIVE" : "BULK_ARCHIVE";
    ...
    <div className="flex-1 flex flex-col overflow-y-auto p-6 gap-6">
      ...
      <div className="flex-shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <IoRefreshOutline
              className={`text-base ${autoRefresh ? "animate-spin" : ""}`}
            />
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:border-indigo-300 transition-all text-sm font-medium"
          >
                ...
                <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-slate-900 rounded-2xl border border-indigo-100 dark:border-indigo-900/40 z-50 overflow-hidden shadow-[0_4px_0_0_rgba(0,0,0,0.05)]">
                  {[
                    ...
                    <button
                      key={fmt}
                      onClick={() => {
                        handleExport(fmt as "csv" | "pdf").then((result) => {
                          showToast(
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          <div
            key={title}
            className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 flex items-center gap-4 shadow-[0_2px_0_0_rgba(0,0,0,0.03)]"
          >
            <div
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:border-indigo-500"
              />
            </div>
            <select
              value={statusFilter}
              ...
              className="px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:border-indigo-500"
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium border transition-all ${showFilters ? "bg-indigo-600 border-indigo-600 text-white" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-indigo-500"}`}
            >
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 rounded-xl text-sm outline-none focus:border-indigo-500"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-indigo-600 mb-1.5">
          ...
          <div className="flex-shrink-0 px-5 py-3 bg-indigo-50 dark:bg-indigo-950/20 border-b border-indigo-100 dark:border-indigo-900/30 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg border border-indigo-300 dark:border-indigo-700 text-indigo-700 dark:text-indigo-400 text-xs font-medium hover:bg-indigo-100 dark:hover:bg-indigo-950/30 transition-colors"
              >
                Désélectionner tout
              </button>
            </div>
            ...
            <div className="flex flex-col items-center justify-center h-80 gap-4">
              ...
              <div className="text-center">
                <p className="text-base font-semibold text-slate-600">
                  {hasActiveFilters ? t("emptySearch") : t("emptyProperties")}
                </p>
                <p className="text-sm text-slate-400 mt-1">
                  {hasActiveFilters
                    ? t("tryDifferentFilters")
                    : t("emptyPropertiesDesc")}
                </p>
                {hasActiveFilters && (
                  <button
                    onClick={resetFilters}
                    className="mt-4 px-4 py-2 text-sm text-indigo-600 font-medium border border-indigo-200 rounded-lg hover:bg-indigo-50"
                  >
                    {t("resetFilters")}
                  </button>
                )}
                ...
                <tr className="border-b border-slate-100 dark:border-slate-800">
                  <th className="px-4 py-3 w-10">
                    ...
                    >
                      ...
                      listings.length > 0 ? (
                        <IoCheckboxOutline className="text-indigo-600" />
                      ) : (
                        <IoSquareOutline className="text-slate-400" />
                      )}
                    </button>
                  </th>
                  <th
                    className="px-4 py-3 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-indigo-600"
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {listing.status === "PENDING_REVIEW" && (
                            <>
                              <button
                                onClick={() => {
                                  handleAction("valider", listing.id).then(
                                    (result) => {
                                      if (result.message)
                                        showToast(
                                          result.success ? "success" : "error",
                              ...
                              <button
                                onClick={() => {
                                  handleAction("rejeter", listing.id).then(
                                    (result) => {
                                      if (result.message)
                                        showToast(
                                          result.success ? "success" : "error",
                          ...
                          {(listing.status === "INACTIVE" ||
                            ...
                            <button
                              onClick={() =>
                                handleActionWithConfirm("activer", listing.id)
                              }
                              className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg"
                              title={t("activateListing")}
                            >
                              <IoCheckmarkCircleOutline className="text-base" />
                            </button>
                          )}
                          ...
                          {(listing.status === "ACTIVE" ||
                            ...
                            <button
                              onClick={() =>
                                handleActionWithConfirm("archiver", listing.id)
                              }
                              className="p-1.5 text-slate-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg"
                              title={t("archiveListing")}
                            >
                              <IoArchiveOutline className="text-base" />
                            </button>
                          )}
                        </div>
                      </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="h-80 rounded-xl overflow-hidden">
            {mapListings.length > 0 ? (
              <MapPickerWrapper
                ...
                showAllMarkers
                onMarkerClick={(id) =>
                  window.open(`/${locale}/listings/${id}`, "_blank")
                }
              />
            ) : (
              <div className="w-full h-full flex flex-col items-center justify-center gap-3 rounded-xl bg-slate-50 border-2 border-dashed border-slate-200">
                <div className="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center">
                  <IoMapOutline className="text-3xl text-slate-400" />
                </div>
                <p className="text-sm text-slate-500 font-medium">
                  {t("mapNoProperties")}
                </p>
                <p className="text-xs text-slate-400">{t("emptyMapDesc")}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 shadow-[0_2px_0_0_rgba(0,0,0,0.03)]">
            ...
            {!recentAlerts || recentAlerts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center mb-3">
                  <IoAlertCircleOutline className="text-3xl text-slate-400" />
                </div>
                <p className="text-sm text-slate-500 font-medium">
                  {t("alertsNoData")}
                </p>
              ...
              displayedAlerts?.map((alertItem, idx) => (
                <div
                  ...
                  className="flex gap-3 p-3 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors cursor-pointer group"
                  onClick={() => {
                    if (alertItem.listingId)
                      window.open(
                        `/${locale}/listings/${alertItem.listingId}`,
                        "_blank",
                      );
                  }}
                >
                  <div className="w-9 h-9 rounded-full bg-sky-100 flex items-center justify-center flex-shrink-0 text-base transition-transform group-hover:scale-105">
                    <IoTimeOutline className="text-sky-600" />
                  </div>
          ...
          {hasMoreAlerts && (
            <button
              onClick={() => setShowAllAlertsState(!showAllAlertsState)}
              className="w-full mt-4 py-2.5 border border-slate-200 text-indigo-500 text-xs font-bold rounded-xl hover:bg-indigo-50 transition-colors flex items-center justify-center gap-2"
            >
              {showAllAlertsState ? (
                <>
