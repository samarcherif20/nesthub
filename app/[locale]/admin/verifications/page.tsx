...
export default function VerificationsPage({ params }: { params: Promise<{ locale: string }> }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center p-10 bg-white dark:bg-slate-900 rounded-2xl shadow-lg">
          <RiUserShieldLine className="text-6xl text-red-400 mx-auto mb-4" />
          <h1 className="text-2xl font-bold text-red-600 mb-2">Accès non autorisé</h1>
          <p className="text-slate-500 dark:text-slate-400">Vous devez être administrateur.</p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto p-6 gap-6">
      ...
      {/* ── HEADER — title left, stats cards right (original layout) ── */}
      <div className="flex-shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        {/* Stats cards — small, in header, like original */}
      ...
      <div className={`flex-1 flex flex-col bg-white dark:bg-slate-900 rounded-2xl border border-indigo-100 dark:border-indigo-900/40 overflow-hidden ${block3d}`}>
        ...
        {/* Table header — search + refresh button here */}
        {requests.length > 0 && (
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <table className="w-full text-left">
              <thead className="sticky top-0 z-10">
                <tr className="bg-indigo-50/50 dark:bg-indigo-900/10 border-b border-indigo-100 dark:border-indigo-900/30">
                  {["Utilisateur", "Rôle", "Date", "Statut", "Action"].map((h, i) => (
            ...
            /* Empty State — original structure preserved */
                ...
                <div className="mt-12 flex items-center gap-8 py-4 px-8 bg-white dark:bg-slate-900 rounded-2xl border border-indigo-100 dark:border-indigo-900/40 shadow-sm">
                  {stats.processedToday > 0 && (
                    <div className="flex flex-col items-center">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Traités aujourd'hui</span>
                      <span className="text-xl font-black text-indigo-600 dark:text-indigo-400">{stats.processedToday} dossiers</span>
                    </div>
                  )}
                  {stats.averageProcessingTime > 0 && (
                    <>
                      ...
                      <div className="flex flex-col items-center">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Temps moyen</span>
                        <span className="text-xl font-black text-violet-600 dark:text-violet-400">{stats.averageProcessingTime} min</span>
                      </div>
                    </>
                  )}
                </div>
