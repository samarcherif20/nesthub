...
import { IoArrowForwardOutline, IoFilterOutline, IoCloseOutline } from "react-icons/io5";
...
export default function VerificationsPage() {
  ...
  const filteredRequests = requests.filter((req) => {
    ...
    if (documentTypeFilter !== "ALL") {
      const isPassport = req.documentType === "passport" || req.cinData?.documentType === "PASSPORT";
  ...
  const getSortIcon = (field: SortField) => {
    ...
    return sortOrder === "asc" ? <TbArrowUp className="inline ml-1 text-xs" /> : <TbArrowDown className="inline ml-1 text-xs" />;
  };
  ...
  const hasActiveFilters = documentTypeFilter !== "ALL" || statusFilter !== "ALL" || search;
    ...
    <div className="flex-1 flex flex-col overflow-y-auto p-6 gap-6">
      ...
      <div className="flex-shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm mt-0.5">{t("description")}</p>
        </div>
        <div className="flex gap-3 flex-shrink-0">
          {[
            { title: t("stats.pending"), value: stats?.pendingCount ?? 0, Icon: MdOutlinePendingActions, grad: "from-amber-400 to-orange-500", border: "border-amber-100 dark:border-amber-900/40", cls: "text-amber-600 dark:text-amber-400" },
            { title: t("stats.processedToday"), value: stats?.processedToday ?? 0, Icon: MdOutlineFactCheck, grad: "from-indigo-500 to-violet-600", border: "border-indigo-100 dark:border-indigo-900/40", cls: "text-indigo-600 dark:text-indigo-400" },
        ...
        <div className="flex-shrink-0 px-5 py-4 border-b border-indigo-50 dark:border-indigo-900/30 bg-gradient-to-r from-indigo-50/40 to-violet-50/20 dark:from-indigo-900/10 dark:to-violet-900/5">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative flex-1 min-w-[200px]">
              ...
              />
              {tempSearch && (
                <button onClick={handleSearchClear} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
            ...
            {hasActiveFilters && (
              <button onClick={resetAllFilters} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors">
                <IoCloseOutline className="text-sm" />
                {t("resetFilters")}
              </button>
            )}
            
            <button onClick={refresh} className="flex items-center gap-2 px-3 py-2 rounded-xl border border-indigo-200 dark:border-indigo-800 bg-white dark:bg-slate-800 text-indigo-500 hover:border-indigo-400 hover:text-indigo-700 transition-all text-sm font-medium">
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 px-4">
              ...
              <h3 className="text-3xl font-extrabold text-slate-900 dark:text-white mb-4 tracking-tight text-center">{t("empty.title")}</h3>
              <p className="text-lg text-slate-500 dark:text-slate-400 mb-10 max-w-lg leading-relaxed text-center">{t("empty.description")}</p>
              <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                <Link href={`/${locale}/admin/dashboard`} className="flex items-center justify-center gap-2 px-8 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-600 hover:to-violet-700 text-white font-bold text-sm shadow-sm transition-all">
                  <TbLayoutDashboard size={18} />
                  {t("empty.backToDashboard")}
                </Link>
                <Link href={`/${locale}/admin/verifications/history`} className="flex items-center justify-center gap-2 px-8 py-3 rounded-xl border border-indigo-200 dark:border-indigo-800 bg-white dark:bg-slate-900 text-indigo-600 dark:text-indigo-400 font-bold text-sm hover:border-indigo-400 transition-all">
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 z-10 bg-indigo-50/50 dark:bg-indigo-900/10">
                <tr className="border-b border-indigo-100 dark:border-indigo-900/30">
                  <th className="px-5 py-3 text-left text-[10px] font-bold text-indigo-400 dark:text-indigo-500 uppercase tracking-wider cursor-pointer hover:text-indigo-600" onClick={() => handleSort("user")}>
                    {t("table.user")} {getSortIcon("user")}
                  </th>
                  <th className="px-5 py-3 text-left text-[10px] font-bold text-indigo-400 dark:text-indigo-500 uppercase tracking-wider">{t("table.role")}</th>
                  <th className="px-5 py-3 text-left text-[10px] font-bold text-indigo-400 dark:text-indigo-500 uppercase tracking-wider">{t("table.documentType")}</th>
                  <th className="px-5 py-3 text-left text-[10px] font-bold text-indigo-400 dark:text-indigo-500 uppercase tracking-wider cursor-pointer hover:text-indigo-600" onClick={() => handleSort("date")}>
                    {t("table.date")} {getSortIcon("date")}
                  </th>
                  <th className="px-5 py-3 text-left text-[10px] font-bold text-indigo-400 dark:text-indigo-500 uppercase tracking-wider cursor-pointer hover:text-indigo-600" onClick={() => handleSort("status")}>
                    {t("table.status")} {getSortIcon("status")}
                  </th>
                  <th className="px-5 py-3 text-left text-[10px] font-bold text-indigo-400 dark:text-indigo-500 uppercase tracking-wider">{t("table.action")}</th>
                </tr>
              </thead>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full overflow-hidden bg-gradient-to-br from-indigo-100 to-violet-100 dark:from-indigo-900/40 dark:to-violet-900/40 flex-shrink-0">
                            {req.user.profilePictureUrl ? (
                              <img src={`/api/admin/serve-image?url=${encodeURIComponent(req.user.profilePictureUrl)}`} alt={req.user.firstName || ""} className="w-full h-full object-cover" />
                            ) : (
                          ...
                          <div>
                            <p className="font-semibold text-slate-800 dark:text-slate-200 text-sm leading-tight">{req.user.firstName} {req.user.lastName}</p>
    ...
    {req.user.role === "PROPERTY_OWNER" ? t("role.owner") : t("role.tenant")}
                      ...
                      <td className="px-5 py-3.5">
                        <span className={`inline-flex text-xs font-semibold px-2.5 py-1 rounded-full ${
                          ...
                          {isPassport ? t("document.passport") : t("document.cin")}
                        </span>
                       </td>
                      <td className="px-5 py-3.5">
                        <span className="text-sm text-slate-500 dark:text-slate-400">{formatRelativeTime(req.submittedAt)}</span>
                       </td>
                      <td className="px-5 py-3.5">
                        {req.status === "PENDING" ? (
                      ...
                      <td className="px-5 py-3.5">
                        <Link href={`/${locale}/admin/verifications/${req.id}`} className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-600 hover:to-violet-700 text-white text-xs font-semibold shadow-sm transition-all">
                          {t("processRequest")}
