        ...
        <div className="w-[38%] shrink-0 flex flex-col overflow-hidden border-r border-slate-200 dark:border-slate-800">
          ...
          <div className="shrink-0 flex items-center justify-between gap-2 px-3 py-2
            ...
            <div className="flex items-center gap-0.5 bg-slate-100 dark:bg-slate-800 rounded-lg p-0.5">
              {(["front", "back"] as const).map((s) => (
                <button
                  key={s}
                  onClick={() => { if (s === "back" && !hasBack) return; setSide(s); }}
            ...
            <div className="flex items-center gap-0.5">
              <button onClick={() => setScale((p) => Math.max(p - 0.15, 0.3))} title={t("zoomOut")}
              ...
              <button onClick={() => setScale((p) => Math.min(p + 0.15, 2.5))} title={t("zoomIn")}
                className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button onClick={() => setRotation((p) => (p - 90 + 360) % 360)} title={t("rotateLeft")}
                className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
                <MdOutlineRotateLeft className="text-base" />
              </button>
              <button onClick={() => setRotation((p) => (p + 90) % 360)} title={t("rotateRight")}
                className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
                <MdOutlineRotateRight className="text-base" />
              </button>
              <button onClick={() => setFlipped((p) => !p)} title={t("flip")}
              ...
              <button onClick={download} title={t("download")}
              ...
              <button onClick={() => currentUrl && setFullscreen(currentUrl)} title={t("fullscreen")}
                className="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-all">
          ...
          <div className="flex-1 flex items-center justify-center overflow-hidden min-h-0
            ...
            {currentUrl ? (
              ...
              >
                <div style={{ transform: flipped ? "scaleX(-1)" : "scaleX(1)", transition: "transform 0.35s ease" }}>
                  <img
                    src={pip(currentUrl)}
                    alt={`CIN ${side}`}
                    draggable={false}
              ...
              <div className="flex items-start gap-5 mt-6">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-slate-500 mt-0.5">
                    {t("memberSince")}{" "}
                    {new Date(requestUser?.createdAt || Date.now()).toLocaleDateString("fr-FR", {
                      month: "long",
                      year: "numeric",
                    })}{" "}
  ...
  <div className="grid grid-cols-2 gap-3">
    ...
    <div className="space-y-0.5 col-span-2">
      ...
      <div className="px-4 py-2 bg-white dark:bg-slate-800 border-2 border-indigo-500/20 rounded-lg text-slate-900 dark:text-slate-100 font-medium text-sm flex items-center gap-3">
        ...
        />
              ...
              <section className="space-y-1.5">
                <label className="text-xs font-medium text-slate-500 uppercase">{t("internalNote")}</label>
                <textarea
                  value={adminComment}
      ...
      <div className="fixed top-12 right-9 z-50 flex flex-col gap-3">
        ...
        {success && <Alert type="success" message={success} onClose={resetSuccess} />}
      </div>
