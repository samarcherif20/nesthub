...
export default function VerificationDetailPage({
  ...
  }, [isAdmin, requestId, fetchRequest]);
  ...
  if (isAdmin === false) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">{t("unauthorized")}</div>
      </div>
    );
  }
  ...
  if (!request) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">{t("requestNotFound")}</div>
      </div>
    );
  }
      ...
      <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between px-6">
        <div className="flex items-center gap-4">
          ...
          <nav className="flex items-center gap-2 text-sm">
            <span className="text-slate-500">{t("queue")}</span>
            <MaterialIcon name="chevron_right" />
            <span className="font-medium">{t("request")} #{requestId.slice(-5)}</span>
          </nav>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                <span className="text-xs font-bold uppercase text-slate-500">{t("cinFront")}</span>
              ...
              <div className="border-2 border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                <img src={request.documentFrontUrl} alt={t("cinFront")} className="w-full object-contain" />
              </div>
            </div>
            ...
            {request.documentBackUrl && (
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-xs font-bold uppercase text-slate-500">{t("cinBack")}</span>
                  <a href={request.documentBackUrl} target="_blank" className="text-xs text-primary hover:underline">
                    {t("download")}
                  </a>
                </div>
                <div className="border-2 border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                  <img src={request.documentBackUrl} alt={t("cinBack")} className="w-full object-contain" />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-8">
            <div className="max-w-xl mx-auto space-y-8">
              ...
              <div className="flex gap-6">
                ...
                <div>
                  ...
                  <p className="text-slate-500 mt-1">
                    {t("memberSince")} {new Date(requestUser?.createdAt).toLocaleDateString()}
                  </p>
                </div>
              </div>
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {extractedData.firstName && (
                    <div>
                      <label className="text-xs text-slate-500">{t("firstName")}</label>
                  ...
                  {extractedData.lastName && (
                    <div>
                      <label className="text-xs text-slate-500">{t("lastName")}</label>
                      <div className="mt-1 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg font-medium">
                        {extractedData.lastName}
                      </div>
                    </div>
                  ...
                  {extractedData.cinNumber && (
                    <div className="col-span-2">
                      <label className="text-xs text-slate-500">{t("cinNumber")}</label>
                  ...
                  {extractedData.dateOfBirth && (
                    <div>
                      <label className="text-xs text-slate-500">{t("dateOfBirth")}</label>
                  ...
                  {extractedData.placeOfBirth && (
                    <div>
                      <label className="text-xs text-slate-500">{t("placeOfBirth")}</label>
                      <div className="mt-1 p-3 bg-slate-50 rounded-lg">
                        {extractedData.placeOfBirth}
                      </div>
                    </div>
                  ...
                  {extractedData.nationality && (
                    <div>
                      <label className="text-xs text-slate-500">{t("nationality")}</label>
                      <div className="mt-1 p-3 bg-slate-50 rounded-lg">
                        {extractedData.nationality}
                      </div>
                    </div>
                  ...
                  {extractedData.expiryDate && (
                    <div>
                      <label className="text-xs text-slate-500">{t("expiryDate")}</label>
                      <div className="mt-1 p-3 bg-slate-50 rounded-lg">
              ...
              <section className="space-y-4 pt-6 border-t border-slate-200">
                <h3 className="text-lg font-bold">{t("finalizeVerification")}</h3>
                ...
                {/* Internal note */}
                <div>
                  <label className="text-sm font-medium">{t("internalNote")}</label>
                  <textarea
                    value={adminComment}
                  ...
                  <div className="p-4 bg-rose-50 dark:bg-rose-900/10 rounded-xl space-y-3">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {[
                        ...
                        t("rejectionReasons.nameMismatch"),
                        t("rejectionReasons.photoNotCompliant"),
                      ].map((motif) => (
                        <button
                          key={motif}
