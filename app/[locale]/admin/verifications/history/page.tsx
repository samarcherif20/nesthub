...
const DateCell = ({ date }: { date?: string }) => {
  if (!date) return <span className="text-sm text-slate-400 dark:text-slate-600">—</span>;
...
export default function VerificationsHistoryPage({
  ...
  const processed    = (globalStats?.validatedCount ?? 0) + (globalStats?.rejectedCount ?? 0);
  const approvalRate = processed > 0
  ...
  const filterTabs: {
  ...
  }[] = [
    {
      ...
      active: "bg-gradient-to-r from-indigo-500 to-violet-600 text-white border-transparent",
    },
    {
      key:    "VALIDATED",
      label:  t("filters.validated"),
      count:  globalStats?.validatedCount ?? 0,
      active: "bg-gradient-to-r from-emerald-500 to-teal-500 text-white border-transparent",
    },
    {
      key:    "REJECTED",
      label:  t("filters.rejected"),
      count:  globalStats?.rejectedCount  ?? 0,
      active: "bg-gradient-to-r from-rose-500 to-red-500 text-white border-transparent",
    },
  ];
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto p-6 gap-6">
      ...
      <div className="shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-indigo-200 dark:border-indigo-800 bg-white dark:bg-slate-900 text-indigo-500 hover:border-indigo-400 hover:text-indigo-700 transition-all text-sm font-medium"
          >
          ...
          <div className="flex items-center gap-2">
            <TbHistoryToggle className="text-indigo-400 text-xl" />
            <div>
        ...
        <div className="flex gap-3 shrink-0">
          {stats.map(({ label, value, Icon, grad, border, cls, ...rest }) => (
            ...
            >
              <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${grad} flex items-center justify-center shadow-sm shrink-0`}>
                <Icon className="text-white text-base" />
              </div>
              <div className="min-w-0 flex-1">
                <p className={`text-xl font-black leading-none ${cls}`}>{value}</p>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5 font-medium truncate">{label}</p>
      ...
      {/* ══ AUDIT BANNER ════════════════════════════════════════════════ */}
      <div className={`shrink-0 flex items-center gap-4 px-5 py-3.5 rounded-2xl border border-indigo-100 dark:border-indigo-900/40 bg-gradient-to-r from-indigo-50/70 to-violet-50/40 dark:from-indigo-900/15 dark:to-violet-900/10 ${card3d}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 truncate">
            NESTHUB maintient une piste d'audit chiffrée de toutes les activités conformément aux normes réglementaires.
          </p>
        </div>
                  ...
                  {TABLE_HEADS.map((h, i) => (
                    <th
                      ...
                      className={`px-5 py-3 text-[10px] font-bold text-indigo-400 dark:text-indigo-500 uppercase tracking-wider whitespace-nowrap ${
                        i === TABLE_HEADS.length - 1 ? "text-right" : "text-left"
                      }`}
                    >
                      {h}
                    </th>
        ...
        {totalPages > 1 && (
          <div className="shrink-0 border-t border-indigo-50 dark:border-indigo-900/30 px-5 py-3 flex items-center justify-between bg-gradient-to-r from-indigo-50/30 to-violet-50/10 dark:from-indigo-900/5 dark:to-violet-900/5">
            <p className="text-xs text-slate-400 dark:text-slate-500">
              ...
              <span className="font-semibold text-slate-600 dark:text-slate-400">{requests.length}</span>
              {" "}{t("pagination.of")}{" "}
              <span className="font-semibold text-slate-600 dark:text-slate-400">{totalItems}</span>
