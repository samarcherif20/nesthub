...
export default function AdminUserDetailsPage() {
  ...
  const isPropertyOwner =
    user?.role === "PROPERTY_OWNER" || user?.role === "BOTH";
  ...
  const goToUserListings = () =>
    router.push(`/admin/properties?userId=${userId}`);
  const goToUserBookings = () =>
    router.push(`/admin/transactions?userId=${userId}`);
  const goToUserDisputes = () =>
    router.push(`/admin/disputes?userId=${userId}`);
  ...
  const getDocumentImage = () => {
    if (isPassport) {
      ...
      return docSide === "front"
        ? user?.cinData?.frontImageUrl
        : user?.cinData?.backImageUrl;
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-indigo-50 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link
            href="/admin/users"
            className="mt-4 text-indigo-600 hover:underline inline-block"
          >
            Retour à la liste
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen">
      <main className="md:ml-2 min-h-screen pb-20">
        <div className="px-4 md:px-8 py-6 md:py-8 space-y-6 md:space-y-8 max-w-[1600px] mx-auto">
          ...
          <div className="grid grid-cols-12 gap-4 md:gap-6">
            ...
            <div
              className={`col-span-12 lg:col-span-8 bg-white dark:bg-slate-900 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row gap-6 md:gap-8 items-start relative overflow-hidden ${block3d}`}
            >
                ...
                <div className="w-24 h-24 md:w-32 md:h-32 rounded-2xl overflow-hidden ring-4 ring-indigo-100 dark:ring-indigo-950/50 group-hover:ring-indigo-300 transition-all">
                  {user.profilePictureUrl ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-3xl md:text-4xl font-bold">
                      {(user.fullName || user.firstName || user.email || "U")
                        .charAt(0)
                        .toUpperCase()}
                    </div>
                  )}
              ...
              <div className="flex-1 space-y-3 md:space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 font-medium text-xs flex items-center gap-2">
                    <IoCalendarOutline className="text-sm" /> Membre depuis{" "}
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-[10px] md:text-xs font-bold text-slate-600 dark:text-slate-300 flex items-center gap-1.5">
                    <FaMedal className="text-[12px] md:text-[14px]" />{" "}
                    {formatUserRole(user.role)}
                  </span>
                  {user.twoFactorEnabled && (
                    <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-[10px] md:text-xs font-bold text-slate-600 dark:text-slate-300 flex items-center gap-1.5">
                      <IoShieldCheckmarkOutline className="text-[12px] md:text-[14px]" />{" "}
                      Double authentification
                    </span>
                  )}
                  {user.governorate && (
                    <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-[10px] md:text-xs font-bold text-slate-600 dark:text-slate-300 flex items-center gap-1.5">
                      <IoLocationOutline className="text-[12px] md:text-[14px]" />{" "}
                      {user.governorate}
                    </span>
                  )}
                  {user.isIdentityVerified && (
                    <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-950/30 rounded-lg text-[10px] md:text-xs font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                      <FaUserCheck className="text-[12px] md:text-[14px]" />{" "}
                      Identité vérifiée
                    </span>
                  )}
                </div>
            ...
            <div className="col-span-12 lg:col-span-4 grid grid-cols-2 gap-3 md:gap-4">
              <div
                className={`bg-white dark:bg-slate-900 rounded-2xl p-4 md:p-6 flex flex-col justify-between ${card3d}`}
              >
                <p className="text-[9px] md:text-xs font-bold text-slate-500 uppercase tracking-widest">
                  Score de confiance
                </p>
                <div>
                  <p className="text-2xl md:text-4xl font-display font-bold text-indigo-600">
                    {trustScore}
                    <span className="text-sm md:text-xl font-medium opacity-50">
                      %
                    </span>
                  </p>
              ...
              >
                ...
                <div>
                  <p className="text-2xl md:text-4xl font-display font-bold text-purple-600">
                    #{user.stats?.globalRank || 142}
                  </p>
                  <p className="text-[9px] md:text-[10px] text-emerald-600 font-bold mt-1 md:mt-2">
                    ↑ 12 positions ce mois
                  </p>
                </div>
              </div>
              <div
              ...
              >
                ...
                <div>
                  <p className="text-xl md:text-2xl font-display font-bold">
                    {Math.round(user.stats?.totalVolume || 0).toLocaleString()}
                    <span className="text-xs md:text-sm font-medium opacity-50">
                      {" "}
                      TND
                    </span>
                  </p>
                  <p className="text-[8px] md:text-[10px] text-slate-400 mt-1 md:mt-2">
                    Valeur brute historique
                  </p>
                </div>
              </div>
              <div
                className={`bg-white dark:bg-slate-900 rounded-2xl p-4 md:p-6 flex flex-col justify-between border-2 border-indigo-200 dark:border-indigo-800 ${card3d}`}
              >
                <p className="text-[9px] md:text-xs font-bold text-indigo-600 uppercase tracking-widest">
                  Niveau de risque
                </p>
                <div>
                  <p
                    className={`text-2xl md:text-4xl font-display font-bold ${riskColor}`}
                  >
                    {formatRiskLevel(user.stats?.trustScore || 85)}
                  </p>
                  <p className="text-[8px] md:text-[10px] text-indigo-600/70 font-bold mt-1 md:mt-2">
                    Dernier audit : il y a 2h
                  </p>
                </div>
              </div>
            </div>
              ...
              {isPropertyOwner ? (
                <>
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-2">
                    <div>
                      <h4 className="text-lg md:text-xl font-display font-bold text-slate-900 dark:text-white">
                        Performance financière
                      </h4>
                      <p className="text-xs md:text-sm text-slate-500">
                        Analyse des revenus et santé des paiements
                      </p>
                    </div>
                    <button
                      onClick={goToUserBookings}
                      className="text-[10px] md:text-xs font-bold text-indigo-600 flex items-center gap-1"
                    >
                      Voir le grand livre{" "}
                  ...
                  <div
                    className={`bg-white dark:bg-slate-900 rounded-2xl p-5 md:p-8 ${card3d}`}
                  >
                    <div className="grid grid-cols-3 gap-4 md:gap-8 mb-6 md:mb-8">
                      <div className="space-y-1">
                        ...
                        <p className="text-xl md:text-2xl font-display font-bold text-slate-900 dark:text-white">
                          {Math.round(
                            user.stats?.activeBalance || 0,
                          ).toLocaleString()}{" "}
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xl md:text-2xl font-display font-bold text-purple-600">
                          {Math.round(
                            user.stats?.pendingPayout || 0,
                          ).toLocaleString()}{" "}
                          <span className="text-[10px] md:text-xs opacity-50">
                            TND
                          </span>
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xl md:text-2xl font-display font-bold text-indigo-600">
                          {Math.round(
                            user.stats?.totalCommission || 0,
                          ).toLocaleString()}{" "}
                          <span className="text-[10px] md:text-xs opacity-50">
                            TND
                          </span>
                    ...
                    <div className="relative h-36 md:h-48 w-full flex items-end gap-1 md:gap-2">
                      {[40, 65, 50, 85, 95, 70, 60, 45, 90, 75, 65, 55].map(
                        (height, i) => (
                          <div
                            key={i}
                            className={`flex-1 ${i === 4 ? "bg-indigo-400" : "bg-slate-200 dark:bg-slate-700"} rounded-t-lg transition-all duration-500 hover:bg-indigo-300 cursor-help`}
                            style={{ height: `${height}%` }}
                          ></div>
                        ),
                      )}
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <div
                      className={`bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 space-y-3 min-h-[210px] ${card3d}`}
                    >
                      {bookings && bookings.length > 0 ? (
                        bookings
                          .slice(0, 3)
                          .map((booking: Booking, idx: number) => (
                            <div
                              key={idx}
                            ...
                            >
                              ...
                              <div className="text-right">
                                ...
                                <span
                                  className={`text-[8px] px-1.5 py-0.5 rounded-full ${booking.status === "COMPLETED" ? "bg-green-100 text-green-600" : booking.status === "PENDING" ? "bg-amber-100 text-amber-600" : "bg-slate-100 text-slate-500"}`}
                                >
                ...
                <div
                  className={`bg-white dark:bg-slate-900 rounded-2xl p-8 text-center ${card3d}`}
                >
                  <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center mx-auto mb-4">
                    <IoReceiptOutline className="text-2xl text-slate-400" />
                  </div>
                  <h4 className="text-lg font-display font-bold text-slate-700 dark:text-slate-300">
                    Aucune donnée financière
                  </h4>
                  <p className="text-xs text-slate-500 mt-2">
                    Cet utilisateur est un voyageur et ne possède pas de données
                    financières d'hôte.
                  </p>
                  <button
                    onClick={goToUserBookings}
                    className="mt-4 text-[10px] md:text-xs font-bold text-indigo-600 flex items-center gap-1"
                  >
            ...
            <div className="col-span-12 xl:col-span-5 space-y-5 md:space-y-6">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-xs md:text-sm text-slate-500">
                    Pièces d'identité et conformité
                  </p>
                </div>
                <button
                  onClick={goToUserVerification}
                  className="text-[10px] md:text-xs font-bold text-indigo-600 flex items-center gap-1"
                >
                  Voir les détails{" "}
                  <IoChevronForwardOutline className="text-sm" />
                ...
                <span
                  className={`px-2 py-1 rounded-md text-[9px] md:text-[10px] font-bold uppercase tracking-wider ${user.isIdentityVerified ? "bg-green-100 dark:bg-green-950/30 text-green-700 dark:text-green-400" : "bg-amber-100 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400"}`}
                >
                  {formatVerificationStatus(user.isIdentityVerified)}
                </span>
              </div>
              <div
                className={`bg-white dark:bg-slate-900 rounded-2xl p-5 md:p-6 space-y-5 md:space-y-6 ${card3d}`}
              >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3">
                      ...
                      <p className="text-sm font-bold text-slate-900 dark:text-white">
                        {user.cinData?.cinNumber ||
                          user.cinData?.passportNumber ||
                          "-"}
                      </p>
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3">
                      ...
                      <p className="text-sm font-bold text-slate-900 dark:text-white">
                        {user.cinData?.firstName} {user.cinData?.lastName}
                      </p>
                    </div>
                ...
                <div className="space-y-3">
                  <p className="text-[9px] md:text-[10px] font-bold text-slate-500 uppercase">
                  ...
                  {hasBack && (
                    <div className="flex gap-2">
                      {["front", "back"].map((s) => (
                        <button
                          key={s}
                          onClick={() => setDocSide(s as "front" | "back")}
                          className={`px-3 py-1 rounded-lg text-[10px] font-bold transition-all ${docSide === s ? "bg-indigo-600 text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300"}`}
                        >
                          {s === "front" ? "RECTO" : "VERSO"}
                        </button>
                      ))}
                  ...
                  <div className="relative">
                    {documentImageUrl ? (
                      <div
                        ...
                        onClick={() => setFullscreenImage(documentImageUrl)}
                      >
                        <img
                          src={getDocumentImageUrl(documentImageUrl)}
                          alt={isPassport ? "Passeport" : "Document d'identité"}
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex items-center justify-center gap-3">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setFullscreenImage(documentImageUrl);
                            }}
                          ...
                          <a
                            href={getDocumentImageUrl(documentImageUrl)}
                            download
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] md:text-xs font-bold text-slate-600 dark:text-slate-300">
                      Avis du modérateur
                    </span>
                  ...
                  <p className="text-[11px] md:text-xs leading-relaxed text-slate-600 dark:text-slate-400 italic font-medium">
                    {user.isIdentityVerified
                      ? "Identité vérifiée dans la base nationale. Aucune anomalie détectée. L'utilisateur est autorisé pour les transactions de grande valeur."
                      : "Vérification des documents en attente. Veuillez examiner les documents téléchargés."}
                  </p>
            ...
            <section className="space-y-5 md:space-y-6">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-xs md:text-sm text-slate-500">
                    Aperçu des annonces de cet hôte
                  </p>
                </div>
                <button
                  onClick={goToUserListings}
                  className="text-[10px] md:text-xs font-bold text-indigo-600 flex items-center gap-1"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {user.listings && user.listings.length > 0 ? (
                  user.listings.map((listing) => (
                    ...
                    >
                      <div
                        className={`group bg-white dark:bg-slate-900 rounded-2xl overflow-hidden hover:-translate-y-1 transition-all duration-300 cursor-pointer ${card3d}`}
                      >
                        <div className="aspect-[4/3] relative">
                          ...
                          <div className="absolute top-3 left-3 px-3 py-1 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-full text-[9px] md:text-[10px] font-extrabold text-indigo-600">
                            PREMIUM
                          </div>
                          <div
                            className={`absolute top-3 right-3 px-3 py-1 rounded-full text-[9px] md:text-[10px] font-extrabold uppercase text-white ${getStatusColor(listing.status)}`}
                          >
                        ...
                        <div className="p-4 md:p-5 space-y-2 md:space-y-3">
                          <div>
                            <h5 className="font-display font-bold text-sm truncate text-slate-900 dark:text-white">
                              {listing.title}
                            </h5>
                  ...
                  <div className="col-span-4 flex flex-col items-center justify-center py-12 md:py-16 text-center">
                    ...
                    <p className="text-xs md:text-sm text-slate-400 dark:text-slate-500 mt-1">
                      Cet hôte n'a pas encore de propriétés
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="col-span-12 lg:col-span-8 space-y-5 md:space-y-6">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-xs md:text-sm text-slate-500">
                    Historique des connexions et interactions en temps réel
                  </p>
                </div>
              </div>
              <div
                className={`bg-white dark:bg-slate-900 rounded-2xl overflow-hidden ${card3d}`}
              >
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse min-w-[600px]">
                    <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                      <tr>
                        <th className="px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[10px] font-bold text-indigo-500 uppercase tracking-widest">
                          Type d'événement
                        </th>
                        <th className="px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[10px] font-bold text-indigo-500 uppercase tracking-widest">
                          Appareil &amp; IP
                        ...
                        <th className="px-4 md:px-6 py-3 md:py-4 text-[9px] md:text-[10px] font-bold text-indigo-500 uppercase tracking-widest text-right">
                          Action
                        </th>
                      </tr>
                    </thead>
                        ...
                        user.recentActivity.slice(0, 5).map((activity) => (
                          ...
                          >
                            <td className="px-4 md:px-6 py-3 md:py-4">
                              <div className="flex items-center gap-2">
                                <div
                                  className={`w-2 h-2 rounded-full ${activity.status === "failed" ? "bg-red-500" : "bg-green-500"}`}
                                ></div>
                            ...
                            <td className="px-4 md:px-6 py-3 md:py-4">
                              <div className="space-y-0.5">
                                <p className="text-[10px] md:text-xs font-bold text-slate-700 dark:text-slate-300">
                                  {activity.device || "Inconnu"}
                                </p>
                            ...
                            <td className="px-4 md:px-6 py-3 md:py-4 text-[10px] md:text-xs font-medium text-slate-600 dark:text-slate-400">
                              {activity.location || "-"}
                            </td>
                            ...
                            <td className="px-4 md:px-6 py-3 md:py-4 text-right">
                              <button
                                onClick={() => viewSessionDetails(activity)}
                                className="text-[9px] md:text-[10px] font-bold text-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity"
                              >
                                DÉTAILS
                              </button>
                            </td>
              ...
              <div className="space-y-4">
                {user.disputes && user.disputes.length > 0 ? (
                  user.disputes.slice(0, 3).map((dispute) => (
                    <div
                      key={dispute.id}
                      className={`bg-white dark:bg-slate-900 rounded-2xl p-4 md:p-5 border-l-4 ${dispute.priority === "high" ? "border-l-red-500" : dispute.priority === "medium" ? "border-l-amber-500" : "border-l-blue-500"} ${card3d}`}
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                          <IoAlertCircleOutline
                            className={`text-sm ${dispute.priority === "high" ? "text-red-500" : dispute.priority === "medium" ? "text-amber-500" : "text-blue-500"}`}
                          />
                          <span
                            className={`px-2 py-0.5 rounded text-[9px] font-extrabold uppercase ${getPriorityColor(dispute.priority)}`}
                          >
                      ...
                      <h6 className="text-[11px] md:text-xs font-bold text-slate-900 dark:text-white mb-1">
                        {dispute.title}
                      </h6>
                      <div className="flex items-center gap-2 mt-2">
                        <span
                          className={`px-2 py-0.5 rounded-full text-[8px] font-bold uppercase ${dispute.status === "open" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"}`}
                        >
                  ...
                  <div
                    className={`bg-white dark:bg-slate-900 rounded-2xl p-8 text-center flex flex-col justify-center items-center min-h-[250px] ${card3d}`}
                  >
                    ...
                    <p className="text-[10px] text-slate-500 mt-1">
                      Cet utilisateur n'a aucun litige ouvert ou résolu
                    </p>
                  </div>
                )}
              </div>
              <button
                onClick={goToUserDisputes}
                className="w-full py-3 md:py-4 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 text-[10px] md:text-xs font-bold text-slate-500 hover:bg-white dark:hover:bg-slate-800 transition-all flex items-center justify-center gap-2"
              >
                <IoTimeOutline className="text-base md:text-[18px]" /> Voir tous
                les litiges
              </button>
            </div>
          </div>
          ...
          >
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${selectedActivity.status === "failed" ? "bg-red-500" : "bg-green-500"}`}
                  ></div>
              ...
              <div className="space-y-4">
                ...
                <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
                  ...
                  <p className="text-sm text-slate-700 dark:text-slate-300 mt-1">
                    {selectedActivity.device || "Inconnu"}
                  </p>
                </div>
                <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                ...
                <div>
                  ...
                  <span
                    className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${selectedActivity.status === "failed" ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400" : "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"}`}
                  >
              ...
              <div className="mt-6 flex justify-end">
                <button
                  onClick={() => setShowSessionModal(false)}
                  className="px-4 py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-sm font-semibold hover:opacity-90 transition"
                >
                  Fermer
                </button>
              </div>
      ...
      {fullscreenImage && (
        ...
        >
          <img
            src={getDocumentImageUrl(fullscreenImage)}
