...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    const handleEsc = (e: KeyboardEvent) => {
      if (e.key === "Escape")
        setMenuPosition({ top: 0, left: 0, userId: null });
    };
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 flex-shrink-0">
          {[
            ...
            <div
              key={title}
              className={`bg-white dark:bg-slate-900 rounded-2xl border ${bg} p-4 flex items-center gap-4 ${card3d}`}
            >
              <div
                    ...
                    <td className="px-4 py-3.5">
                      ...
                      {user.phoneNumber && (
                        <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                          <BsTelephone className="text-[10px]" />
                          {user.phoneNumber}
                        </p>
                      )}
                    </td>
                  ...
                  <div className="py-1">
                    ...
                    {u.status === "ACTIVE" && (
                      <>
                        ...
                        >
                          <IoPauseCircleOutline className="text-base text-amber-500" />
                          {t("actions.suspend")}
                        </button>
                      </>
                    )}
                    ...
                    {u.status !== "PERMANENTLY_BANNED" && (
                      <>
                        ...
                        >
                          <IoArrowUpCircleOutline className="text-base text-violet-500" />
                          {t("actions.escalate")}
                        </button>
                      </>
                    )}
