...
export default function ResetPasswordPage() {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        // Maintenant on peut définir le nouveau mot de passe
      ...
      if (err.errors && err.errors.length > 0) {
        ...
        if (clerkError.code === "verification_code_incorrect") {
  ...
  // Si on est à l'étape code, demander d'abord le code
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2">
              ...
              {email?.includes("+clerk_test") && (
                <span className="block mt-1 text-sm text-blue-600">
                  🔧 Mode test : utilisez le code <strong>424242</strong>
                </span>
              )}
            </p>
          </div>
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="000000"
                  className="w-full px-4 py-2.5 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all text-center text-lg tracking-widest"
                  disabled={loading}
                  autoFocus
                />
              </div>
