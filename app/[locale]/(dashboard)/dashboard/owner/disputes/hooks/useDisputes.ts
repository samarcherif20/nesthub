...
export function useDisputes(t: any): UseDisputesReturn {
  ...
  const activeFilters = (statusFilter !== "all" ? 1 : 0) + (priorityFilter !== "all" ? 1 : 0) + (searchQuery ? 1 : 0);

  const filteredDisputes = useMemo(() => {
    let out = disputes.filter((d) => {
      const matchesSearch = !searchQuery || 
        (d.reference || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
        (d.booking.listing.title || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
        (d.booking.tenant.username || "").toLowerCase().includes(searchQuery.toLowerCase());
      const matchesStatus = statusFilter === "all" || d.status === statusFilter;
      const matchesPriority = priorityFilter === "all" || d.priority === priorityFilter;
      return matchesSearch && matchesStatus && matchesPriority;
    });
    const prioRank = { HIGH: 0, MEDIUM: 1, LOW: 2 };
    if (sort === "newest") out.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    else if (sort === "oldest") out.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
