...
function Toast({
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-xl text-white text-sm font-medium shadow-lg animate-in slide-in-from-bottom-2 ${styles[type]}`}
    >
      {message}
    </div>
  );
...
export default function OwnerMessagesPage() {
  ...
  const handleSelectConversation = (conv: Conversation) => {
    ...
    console.log("🔍 [Owner] Offer dans la conversation:", conv.offer);
    console.log("🔍 [Owner] Status de l'offre:", conv.offer?.status);
  ...
  const handleAcceptOffer = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(
          "Offre acceptée avec succès ! La réservation est confirmée.",
          "success",
        );
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                      ...
                      {conv.unreadCount > 0 && (
                        <span className="min-w-[20px] h-5 rounded-full bg-indigo-500 text-white text-[10px] font-bold flex items-center justify-center px-1.5">
                          {conv.unreadCount}
                        </span>
                      )}
                    </div>
              ...
              <div className="rounded-xl overflow-hidden bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-700">
                <div className="h-32 w-full relative">
                  ...
                  {selectedConv.listing.pricePerNight && (
                    <div className="absolute top-2 right-2 bg-white/90 dark:bg-slate-900/90 backdrop-blur px-2 py-1 rounded-lg text-[11px] font-bold text-indigo-600 dark:text-indigo-400 shadow-sm">
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      <Verified className="w-3.5 h-3.5 text-indigo-500" />
              ...
              <button
                ...
                className={`w-full py-3 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
                  selectedConv?.offer?.status === "PENDING" && !isProcessing
                    ? "bg-emerald-600 text-white shadow-lg shadow-emerald-500/20 hover:bg-emerald-700 active:scale-[0.98]"
            ...
            <div className="pt-2 border-t border-slate-200 dark:border-slate-700">
              <div className="flex items-center justify-center gap-1">
                ...
                <p className="text-[10px] text-slate-400 text-center leading-relaxed">
                  En acceptant, vous pré-approuvez la réservation.
                  <br />
                  Le locataire aura 24h pour finaliser le paiement.
                </p>
              </div>
            </div>
          </div>
