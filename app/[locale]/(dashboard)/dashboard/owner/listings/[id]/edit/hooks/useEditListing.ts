...
interface Listing {
  ...
  rooms: number; bathrooms: number; maxGuests: number;
  surfaceArea: number | null; floorNumber: number | null; hasElevator: boolean;
  equipment: Record<string, boolean>; services: Record<string, boolean>;
  houseRules: Record<string, boolean>; customRules: string;
  photos: Array<{ id: string; url: string; thumbnailUrl: string; isMain: boolean; position: number }>;
  rentalType: string; pricePerNight: number | null; pricePerMonth: number | null;
  securityDeposit: number | null; cleaningFee: number | null;
  status: string; viewCount: number; createdAt: string; updatedAt: string;
}
...
export function useEditListing(id: string, locale: string) {
  ...
  const setField = useCallback((field: keyof Listing, value: any) => {
    if (listing) setListing({ ...listing, [field]: value });
  }, [listing]);

  const save = useCallback(async (extraData?: Partial<Listing>) => {
    if (!listing) return;
    setSaving(true);
    try {
      const res = await fetch(`/api/listings/${id}`, {
  ...
  const handleFileChange = useCallback(async (files: FileList | null) => {
    ...
    try {
      const uploaded = [];
      for (const file of Array.from(files).slice(0, 20 - listing.photos.length)) {
        const fd = new FormData();
        fd.append("photos", file);
        const res = await fetch("/api/listings/upload-temp-photo", { method: "POST", body: fd });
        if (res.ok) {
          const data = await res.json();
          uploaded.push({
  ...
  const removePhoto = useCallback((photoId: string) => {
    ...
    if (filtered.length > 0 && !filtered.some(p => p.isMain)) filtered[0].isMain = true;
  ...
  const qualityScore = listing ? Math.min(85 + (listing.photos.length || 0) * 2 + (listing.description?.length || 0) / 100, 98) : 0;
  ...
  return {
    ...
    uploadingPhotos,
    qualityScore,
    alert,           // ✅ AJOUTER CETTE LIGNE
    setAlert,        // ✅ AJOUTER CETTE LIGNE
    setField,
    save,
    handleFileChange,
    removePhoto,
